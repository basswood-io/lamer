                 ...
                 int *max_frame_bits_out)
{
    ...
    if (totbits > max_frame_bits && max_frame_bits > 0) {
        int const actual_total = totbits;
        totbits = 0;
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            for (ch = 0; ch < cfg->channels_out; ch++) {
                targ_bits[gr][ch] *= max_frame_bits;
                targ_bits[gr][ch] /= actual_total;
                totbits += targ_bits[gr][ch];
            }
        }
    }
    ...
    for (gr = 0; gr < cfg->mode_gr; gr++) {
        for (ch = 0; ch < cfg->channels_out; ch++) {
            int const channel_bits = targ_bits[gr][ch];
            targ_bits_out[gr][ch] = channel_bits;
            if (analog_silence_bits > channel_bits)
                analog_silence_bits = channel_bits;
        }
    }
