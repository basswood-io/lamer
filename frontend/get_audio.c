...
static int read_samples_mpeg(lame_t gfp, FILE * musicin, short int pcm[2][1152]);
#ifdef LIBSNDFILE
...
get_audio_common(lame_t gfp, int buffer[2][1152], short buffer16[2][1152])
{
    ...
    if (is_mpeg_file_format(global_reader.input_format)) {
        if (buffer != NULL)
            samples_read = read_samples_mpeg(gfp, global.music_in, buf_tmp16);
        else
            samples_read = read_samples_mpeg(gfp, global.music_in, buffer16);
        if (samples_read < 0) {
            return samples_read;
        }
    }
...
static int
read_samples_mpeg(lame_t gfp, FILE * musicin, short int pcm[2][1152])
{
    (void) gfp;
    (void) musicin;
    (void) pcm;
